import svgApple from '../../assets/icons/apple.svg';
import svgBack from '../../assets/icons/back.svg';
import svgBackBottom from '../../assets/icons/backBottom.svg';
import svgCalendar from '../../assets/icons/calendar.svg';
import svgCheck from '../../assets/icons/check.svg';
import svgEmail from '../../assets/icons/email.svg';
import svgFilter from '../../assets/icons/filter.svg';
import svgGmail from '../../assets/icons/gmail.svg';
import svgList from '../../assets/icons/list.svg';
import svgMore from '../../assets/icons/more.svg';
import svgPlus from '../../assets/icons/plus.svg';
import svgPlus2 from '../../assets/icons/plus_2.svg';
import svgSetting from '../../assets/icons/setting.svg';
import svgSmile from '../../assets/icons/smile.svg';
import svgStudentBlack from '../../assets/icons/student_black.svg';
import svgStudentCheck from '../../assets/icons/studentCheck.svg';
import svgTeacherBlack from '../../assets/icons/teacher_black.svg';
import svgTime from '../../assets/icons/time.svg';
import svgTrash from '../../assets/icons/trash.svg';

export const SvgBack = svgBack;
export const SvgCheck = svgCheck;
export const SvgSmile = svgSmile;
export const SvgBackBottom = svgBackBottom;
export const SvgPlus = svgPlus;
export const SvgPlus2 = svgPlus2;
export const SvgApple = svgApple;
export const SvgGmail = svgGmail;
export const SvgEmail = svgEmail;
export const SvgSetting = svgSetting;
export const SvgStudentBlack = svgStudentBlack;
export const SvgTeacherBlack = svgTeacherBlack;
export const SvgFilter = svgFilter;
export const SvgList = svgList;
export const SvgStudentCheck = svgStudentCheck;
export const SvgTrash = svgTrash;
export const SvgCalendar = svgCalendar;
export const SvgMore = svgMore;
export const SvgTime = svgTime;

export default [
  SvgBack,
  SvgCheck,
  SvgSmile,
  SvgBackBottom,
  SvgPlus,
  SvgPlus2,
  SvgApple,
  SvgGmail,
  SvgEmail,
  SvgSetting,
  SvgStudentBlack,
  SvgTeacherBlack,
  SvgFilter,
  SvgStudentCheck,
  SvgTrash,
  SvgCalendar,
  SvgMore,
  SvgTime,
];
